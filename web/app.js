/* ============================================================
   Shiva Portal — UI layer (backend-agnostic, async via Store)
   ============================================================ */
const AV=["#6d5efc","#22d3ee","#8b5cf6","#ec4899","#f97316","#34d399","#60a5fa","#a855f7"];
let state={users:[],tasks:[],docs:[],research:[],activity:[],sessionUser:null};
let route="dashboard";
...
function toggleTheme(){const cur=localStorage.getItem("shiva_theme")==="light"?"":"light";
  localStorage.setItem("shiva_theme",cur||"dark");applyTheme();const b=document.getElementById("themebtn");if(b)b.textContent=cur==="light"?"☾":"☀";}
...
function showLogin(){
  ...
  const m=document.getElementById("li-mode");m.textContent=Store.mode==="cloud"?"● CLOUD":"● LOCAL";
  m.className="mode "+(Store.mode==="cloud"?"cloud":"local");
...
function showApp(){document.getElementById("login").classList.add("hidden");
  document.getElementById("app").style.display="block";if(!can(route))route="dashboard";renderShell();renderRoute()}
async function doLogin(){
  const u=document.getElementById("li-user").value.trim(),p=document.getElementById("li-pass").value;
  const err=document.getElementById("li-err");err.textContent="Signing in…";
  const r=await Store.login(u,p);if(r.error){err.textContent=r.error;return}
  await Store.addActivity("Signed in");await refresh();showApp()}
...
function renderShell(){
  ...
  const myTasks=state.tasks.filter(t=>t.assignee===u.username&&t.status!=="Done").length;
  ...
  document.getElementById("app").innerHTML=`<div class="shell">
   <aside class="side">
     ...
     <nav><div class="navlbl">WORKSPACE</div>${main.map(item).join("")}
       ${sys.length?`<div class="navlbl">ACCOUNT</div>${sys.map(item).join("")}`:""}</nav>
   ...
   <main class="main">
     <div class="topbar"><div class="crumb">Shiva / <b id="crumb">Dashboard</b></div>
...
function viewDashboard(){
  const u=me();const myTasks=state.tasks.filter(t=>t.assignee===u.username);
  const myDone=myTasks.filter(t=>t.status==="Done").length;
  const allDone=state.tasks.filter(t=>t.status==="Done").length;
  const prog=state.tasks.length?Math.round(allDone/state.tasks.length*100):0;
  ...
  return `<div class="page-h"><div><h1>Welcome back, ${esc(u.name.split(" ")[0])}</h1>
   ...
   <div class="grid c4" style="margin-bottom:16px">
     ...
     ${stat("◎","var(--blue-soft)","var(--blue)","Phase 0","Current phase","Learn + Break","var(--blue)")}
     ${stat("◭","var(--green-soft)","var(--green)",state.users.length,"Team members")}
   </div>
   <div class="grid c3">
     <div class="card" style="grid-column:span 2"><div class="hd"><h3>My tasks today</h3>
     ...
     <div class="card"><div class="hd"><h3>Overall progress</h3></div><div class="bd">
       <div style="font-size:30px;font-weight:700">${prog}%</div>
       <div class="l" style="color:var(--muted);margin:2px 0 12px">${allDone} of ${state.tasks.length} tasks complete</div>
       <div class="pbar"><i style="width:${prog}%"></i></div>${miniRoadmap()}</div></div>
   </div>
   <div class="card" style="margin-top:16px"><div class="hd"><h3>Recent activity</h3>
     ...
     <div class="bd" style="padding-top:6px">${activityList(state.activity.slice(0,6))}</div></div>`;
}
function miniRoadmap(){const phases=[["Phase 0",1],["Phase 1",0],["Phase 2",0],["Phase 3",0]];
  return `<div style="display:flex;gap:8px;margin-top:18px">${phases.map(p=>`<div style="flex:1;text-align:center">
    <div style="height:6px;border-radius:99px;background:${p[1]?'var(--primary)':'var(--surface-3)'}"></div>
    <div style="font-size:10.5px;color:var(--muted);margin-top:6px;font-weight:600">${p[0]}</div></div>`).join("")}</div>`}
  ...
  return `<div class="page-h"><div><h1>Project Roadmap</h1><p>The 18-month plan, by phase, with decision gates.</p></div></div>
   ...
   <div class="grid c2" style="margin-top:16px">
     <div class="card"><div class="hd"><h3>Phase 0 — current focus</h3><span class="badge amber">In progress</span></div>
       <div class="bd"><ul style="margin:0;padding-left:18px;line-height:1.9;color:var(--muted)">
...
function viewTasks(){
  ...
  const list=state.tasks.slice().sort((a,b)=>(a.due||"").localeCompare(b.due||""));
  return `<div class="page-h"><div><h1>Tasks</h1><p>Daily task allocation and tracking.</p></div>
   ...
   <div class="grid c3" style="margin-bottom:16px">${cols.map(c=>{const n=state.tasks.filter(t=>t.status===c[0]).length;
     return `<div class="stat"><div class="v">${n}</div><div class="l">${c[0]}</div>
       <div class="pbar" style="margin-top:10px"><i style="width:${state.tasks.length?n/state.tasks.length*100:0}%"></i></div></div>`}).join("")}</div>
   <div class="card"><table class="tbl"><thead><tr><th>Task</th><th>Assignee</th><th>Due</th><th>Priority</th><th>Status</th><th></th></tr></thead>
   <tbody>${list.map(t=>{const pr={Critical:"red",High:"amber",Medium:"blue",Low:"grey"}[t.priority]||"grey";
...
function viewDocuments(){
  ...
  return `<div class="page-h"><div><h1>Documents</h1><p>Upload and share project files.</p></div>
   ...
   <tbody>${state.docs.map(d=>`<tr data-srch="${esc(d.name)}">
     ...
     <td style="text-align:right">${d.data?`<a class="btn sm" href="${d.data}" target="_blank" download="${esc(d.name)}">Open</a> `:''}
       <button class="btn sm danger" onclick="delDoc('${d.id}')">Delete</button></td></tr>`).join("")}
   </tbody></table>${state.docs.length?'':emptyState("▤","No documents yet. Upload your first file.")}</div>`;
}
function viewResearch(){
  ...
  return `<div class="page-h"><div><h1>Research</h1><p>Curated sources, notes and reading for the project.</p></div>
   ...
   <div class="grid c2">${state.research.map(r=>`<div class="card"><div class="bd">
...
function viewActivity(){return `<div class="page-h"><div><h1>Activity</h1><p>Audit trail of everything happening in the portal.</p></div></div>
   <div class="card"><div class="bd">${activityList(state.activity)}</div></div>`}
...
function viewAdmin(){
  const u=me();if(!["Owner","Admin"].includes(u.role))return emptyState("🔒","You don't have access to user administration.");
  return `<div class="page-h"><div><h1>Admin · Users</h1><p>Create accounts, assign roles and section access.</p></div>
   ...
   <tbody>${state.users.map(x=>`<tr data-srch="${esc(x.name)} ${esc(x.username)} ${x.role}">
     ...
     <td><span class="badge ${x.status==='Active'?'green':'red'}">${x.status}</span></td><td>${fmt(x.created)}</td>
     <td style="text-align:right">${x.username!==state.sessionUser?`<button class="btn sm" onclick="toggleUserStatus('${x.id}')">${x.status==='Active'?'Disable':'Enable'}</button>
   ...
   <div class="foot">${Store.mode==='cloud'?'Cloud mode — real accounts via Supabase.':'Prototype — accounts live in this browser only. Connect Supabase for real cross-device login (docs/supabase-setup.md).'}</div>`;
}
   ...
   <div class="grid c2">
     <div class="card"><div class="hd"><h3>Account</h3></div><div class="bd">
       <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px">
         ...
         <div><div style="font-weight:700;font-size:17px">${esc(u.name)}</div><div style="color:var(--muted)">@${esc(u.username)} · ${u.role}</div></div></div>
...
/* ============ ACTIONS ============ */
async function toggleTask(id){const t=state.tasks.find(x=>x.id===id);if(!t)return;
  const ns=t.status==="Done"?"To do":"Done";await Store.updateTask(id,{status:ns});
  await Store.addActivity((ns==="Done"?"Completed":"Reopened")+" task: "+t.title);reload()}
async function setTaskStatus(id,s){const t=state.tasks.find(x=>x.id===id);if(!t)return;
  await Store.updateTask(id,{status:s});await Store.addActivity("Set '"+t.title+"' → "+s);reload()}

function openTaskModal(){
  const opts=state.users.filter(u=>u.status==="Active").map(u=>`<option value="${u.username}">${esc(u.name)}</option>`).join("");
  modal("New task",`<label class="field"><span>Title</span><input id="t-title" placeholder="e.g. Reproduce drift attack"></label>
...
function openResearchModal(){
  modal("Add research entry",`<label class="field"><span>Title</span><input id="r-title" placeholder="Source / paper title"></label>
    ...
    "Add entry",async()=>{const title=val("r-title");if(!title)return alert("Title required");
      await Store.createResearch({title,url:val("r-url"),category:val("r-cat"),note:val("r-note")});
      await Store.addActivity("Added research: "+title);closeModal();reload()})}
function genUsername(name){let base=(name.trim().split(/\s+/)[0]||"user").toLowerCase().replace(/[^a-z]/g,"")||"user";
  let u=base,n=1;while(state.users.some(x=>x.username===u)){u=base+n;n++}return u}
function genPassword(){const U="ABCDEFGHJKLMNPQRSTUVWXYZ",l="abcdefghijkmnpqrstuvwxyz",d="23456789",s="!@#$%&*";
  const pk=set=>set[Math.floor(Math.random()*set.length)];let p=[pk(U),pk(U),pk(l),pk(l),pk(l),pk(d),pk(d),pk(d),pk(s),pk(s)];
...
function openUserModal(){
  ...
  modal("Add user",`<label class="field"><span>Full name</span><input id="u-name" placeholder="e.g. Asha Patel"></label>
    ...
    "Create account",async()=>{const name=val("u-name");if(!name)return alert("Name required");
      const access=[...document.querySelectorAll(".acc:checked")].map(c=>c.value);
      const username=genUsername(name),password=genPassword();
      try{await Store.createUser({name,username,password,role:val("u-role"),access});}
      catch(e){return alert("Could not create user: "+(e.message||e))}
      await Store.addActivity("Created account for "+name+" (@"+username+")");closeModal();showCreds(name,username,password)});
...
function showCreds(name,username,password){
  modal("Account created ✓",`<p style="margin:0 0 4px;color:var(--muted)">Share these with <b style="color:var(--ink)">${esc(name)}</b>. The password is shown once — copy it now.</p>
...
async function toggleUserStatus(id){const x=state.users.find(u=>u.id===id);if(!x)return;
  const ns=x.status==="Active"?"Disabled":"Active";await Store.updateUser(id,{status:ns});
  await Store.addActivity((ns==="Active"?"Enabled":"Disabled")+" @"+x.username);reload()}
async function delUser(id){const x=state.users.find(u=>u.id===id);if(!x||!confirm("Remove "+x.name+"?"))return;
  await Store.deleteUser(id);await Store.addActivity("Removed @"+x.username);reload()}

async function uploadDoc(input){const f=input.files[0];if(!f)return;
  const size=(f.size/1024).toFixed(0)+" KB";const cat=f.type.includes("pdf")?"PDF":f.type.includes("image")?"Image":"File";
  if(Store.mode==="cloud"){await Store.createDoc({name:f.name,category:cat,size,file:f});await Store.addActivity("Uploaded document: "+f.name);return reload()}
  const finish=async data=>{await Store.createDoc({name:f.name,category:cat,size,data,by:state.sessionUser});
    await Store.addActivity("Uploaded document: "+f.name);reload()};
  if(f.size<=1500000){const r=new FileReader();r.onload=()=>finish(r.result);r.readAsDataURL(f)}
  else{alert("File over 1.5 MB — stored as a reference only (local prototype limit).");finish(null)}}
...
async function saveProfile(){const n=val("pf-name").trim();if(!n)return;await Store.updateSelf({name:n});
  await Store.addActivity("Updated profile");reload()}
async function changePassword(){const u=me();const cur=val("pf-cur"),nw=val("pf-new");const msg=document.getElementById("pf-msg");
  ...
  await Store.addActivity("Changed password");msg.style.color="var(--green)";msg.textContent="Password updated ✓";
...
async function reseedConfirm(){if(Store.mode!=="local")return alert("Reset is only available in local mode.");
  if(confirm("Reset ALL local portal data (users, tasks, documents)? This cannot be undone.")){await Store.resetAll();await refresh();showApp()}}
...
/* start */
window.addEventListener("keydown",e=>{if(e.key==="Enter"){const lg=document.getElementById("login");
